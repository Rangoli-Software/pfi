...
kim.createRelatedViewer = function (skuInfo, catalog) {
...
kim.createComponentFactory = function (prodInfo, dimensioner, sizer, categorizer) {
    var navHelper = createNavHelper(prodInfo, categorizer, kim.catalog.title);
    //    var relatedviewer = kim.createRelatedViewer(prodInfo.skuInfo, catalog);
...
kim.shippingHTML = getShippingInfoUL(['If the item is in stock,  it will be ready to ship within 1 business day of your order.']);
kim.washcareHTML = cotton_washcareHTML;
...
kim.createJSON = function (style) {
    var basePath = "/products/kim/" + style.imgDir + "/";
    return createProductJSON(style.SKU, basePath, style, null, createIMImageFactory);
}

kim.catalog = {
    title: "It's Magic",
    shopURL: "/products/kim/shop.html",
    skus: null,
    styles: [kim.princess, kim.flow, kim.wave, kim.layer, kim.fairy, kim.gypsy, kim.kidikini, kim.halfpant, kim.fullpant, kim.prince, kim.royal, kim.balloon],
    unisex: [kim.balloon.SKU],
    boys: [kim.halfpant.SKU, kim.fullpant.SKU, kim.prince.SKU, kim.royal.SKU],
    girls: [kim.princess.SKU, kim.flow.SKU, kim.wave.SKU, kim.layer.SKU, kim.fairy.SKU, kim.gypsy.SKU, kim.kidikini.SKU],
    dresses: [],
    tops: [],
    shirts: [],
    pants: [],
...
pfiavG.getLineInitializer(kim).initialize();

kim.categorizer = createFieldCategorizer(
    kim.catalog,
    ["Girls", "Boys", "Both"],
    ["girls", "boys", "unisex"],
    ["g", "b", "u"],
